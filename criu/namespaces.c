...
int cr_enter_ns(pid_t *target_pid) {
    ...
    if (opts.enter_pidns) {
        if (!opts.enter_mntns) {
            /* We strictly require that procfs is available.  Unless we don't
             * mind nuking procfs for the parent pid namespace, we must remount
             * /proc for the child pid namespace in a mount namespace.
             */
            pr_err("Must specify --enter-mntns with --enter-pidns\n");
            exit(1);
        }
